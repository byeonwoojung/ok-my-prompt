...
function mapError(err: unknown) {
  ...
  if (message.includes('billing') || message.includes('quota')) {
    return { code: 'NO_CREDITS', message: 'Google API 할당량이 초과되었습니다.', retryable: false };
  }
  ...
  try {
    ...
    if (!apiKey) {
      return NextResponse.json(
        { error: { code: 'INVALID_KEY', message: 'Google API 키가 설정되지 않았습니다. 설정에서 키를 등록하세요.', retryable: false } },
        { status: 401 }
      );
    }

    const result = await completeGoogle(apiKey, {
      prompt,
      model,
