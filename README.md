...
- **멀티 모델 지원** — OpenAI(GPT-5.4 / GPT-4.1 / GPT-4o / o4-mini / o3 등), Google Gemini(3.x Preview / 2.5 / 1.5), Anthropic Claude(4.6 / 4.5 / 3) 총 35개 모델. 추론 모델(o-series, GPT-5.x)은 자동으로 `max_completion_tokens` 분기 처리.
- **프롬프트 순열 시스템** — `{{$변수이름}}` 플레이스홀더로 모든 옵션 조합을 자동 생성하여 한번에 테스트. `$` 접두사로 JSON 출력 포맷의 literal `{{ }}`와 명확히 구분. 한국어 NFD(자모 분리) 입력 자동 정규화 지원.
- **배치 / 순서 섞기 모드** — 같은 프롬프트를 N회 반복하거나, 플레이스홀더 위치를 바꿔서 순서 효과 테스트 (둘 중 택1). 텍스트 생성 / 이미지 분석 모두 지원.
- **스트리밍 응답** — SSE 기반 실시간 글자 단위 표시
- **이미지 분석** — 이미지 업로드(드래그앤드롭/붙여넣기) + 비전 모델 테스트 + 배치/순서 섞기 모드 지원
